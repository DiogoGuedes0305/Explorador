...
export default class Maze extends THREE.Group {
    constructor(parameters) {
        ...
        this.onLoad = function (description) {
            ...
            // create passages
            description.fmPassages?.forEach((r) => {
                this.passage = null;
    ...
    foundExit1(position) {
        return (Math.abs(position.x - this.exitLocation1.x) < 0.5 * this.scale.x &&
         Math.abs(position.z - this.exitLocation1.z) < 0.5 * this.scale.z);
    }
    foundExit2(position) {
        return (Math.abs(position.x - this.exitLocation2.x) < 0.5 * this.scale.x &&
         Math.abs(position.z - this.exitLocation2.z) < 0.5 * this.scale.z);
    }
    foundExit3(position) {
        return (Math.abs(position.x - this.exitLocation3.x) < 0.5 * this.scale.x &&
         Math.abs(position.z - this.exitLocation3.z) < 0.5 * this.scale.z);
    }
